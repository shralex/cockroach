...
func (env *InteractionEnv) Handle(t *testing.T, d datadriven.TestData) string {
	...
	case "propose-conf-change":
		// Propose a configuration change, or continue a previously proposed change.
		// When adding nodes, this command can be used to logically add nodes to the configuration,
		// but add-nodes is needed to "create" the nodes.
		//
		// propose-conf-change node_id <optional v1> <optional transition>
		// command string
		// See ConfChangesFromString for command string format.
		// Arguments are:
		//    node_id - the node proposing the configuration change
		//    bool v1 (make one change at a time, false by default)
		//    bool transition ("auto" (the default), "explicit" or "implicit")
		// Example:
		//
